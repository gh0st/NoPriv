...
IMAPFOLDER_ORIG = [
    folder.strip()
    for folder in config.get("nopriv", "imap_folder").split(",")
    if folder.strip() != ""
]
...
def saveMostRecentMailID(mail_id, email_address, folder, filename="nopriv.txt"):
    ...
    for line in fileinput.input(filename, inplace=1):
        if (
            line.split(":")[0] == folder
            and line.split(":")[1] == email_address
            and len(line) > 3
        ):
...
def moveMailDir(maildir):
    ...
    now = datetime.datetime.now()
    maildirfilename = "Maildir." + str(now).replace("/", ".").replace(" ", ".").replace(
        "-", "."
    ).replace(":", ".")
...
def finishOverviewPage(
    folder, pagenumber, previouspage, nextpage, total_messages_in_folder
):
    overview_page_name = "email-report-" + str(pagenumber) + ".html"
    overview_file_path = os.path.join(folder, overview_page_name)
    with open(overview_file_path, "a") as overview_file:
...
def createMailPage(
    ...
    print(
        ("Processing mail %s from %s with subject %s.")
    ...
    with open(mail_html_page, "w") as mail_page:
        mail_page.write(
            returnHeader(
                mail_subject + " - NoPriv.py Email Backup by Raymii.org",
                "../../../inc/",
            )
        )
        ...
        if has_attachments:
            mail_page.write("\t<tr>\n")
            mail_page.write(
                '\t\t<td></td><td><a href="attachments">Click here to open the attachments.</a> </td>\n'
            )
            mail_page.write("\t<tr>\n")
        mail_page.write(
...
def save_mail_attachments_to_folders(mail_id, mail, local_folder, folder):
    ...
    with open(
        os.path.join(folder, att_date, str(mail_id), "attachments/index.html"), "w"
    ) as att_index_file:
        att_index_file.write(
            returnHeader(
                "Attachments for mail: " + str(mail_id) + ".", "../../../../inc"
            )
        )
    ...
    for part in mail.walk():
        ...
        att_path = os.path.join(
            folder, att_date, str(mail_id), "attachments", att_filename
        )
    ...
    with open(
        os.path.join(folder, att_date, str(mail_id), "attachments/index.html"), "a"
    ) as att_index_file:
        att_index_file.write("</ul>")
...
def backup_mails_to_html_from_local_maildir(folder):
    ...
    try:
        number_of_overview_pages = float(total_messages_in_folder) / float(
            messages_per_overview_page
        )
    ...
    for number in reversed(range(start_mail_number)):
        ...
        mail_has_attachment = save_mail_attachments_to_folders(
            mail_number, mail_for_page, folder, folder
        )
    ...
    for number in reversed(range(number_of_overview_pages)):
        ...
        if number == 1 and number_of_overview_pages == 1:
            ...
            finishOverviewPage(
                folder, number, 0, (number + 1), total_messages_in_folder
            )
        elif number == number_of_overview_pages:
            finishOverviewPage(
                folder, number, (number - 1), 0, total_messages_in_folder
            )
        else:
            finishOverviewPage(
                folder, number, (number - 1), (number + 1), total_messages_in_folder
            )
